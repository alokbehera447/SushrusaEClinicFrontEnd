...
const ConsultationWorkspace: React.FC = () => {
  ...
  // Medical Records pagination states
  const [medicalRecordsPage, setMedicalRecordsPage] = useState(1);
  const [medicalRecordsPageSize] = useState(10);
  const [medicalRecordsTotalCount, setMedicalRecordsTotalCount] = useState(0);
  const [showAllMedicalRecords, setShowAllMedicalRecords] = useState(false);
        ...
        if (consultData) {
          ...
          if (patientId) {
            ...
            try {
              ...
              const totalCount = recordsData?.count || recordsData?.total || records.length;
              setMedicalRecordsTotalCount(totalCount);
            } catch (error) {
            ...
            try {
              const docsResponse = await api.get(`/api/patients/${patientId}/documents/`, {
                params: { page: 1, page_size: 10 }
              });
              const docsData = docsResponse.data.success ? docsResponse.data.data : docsResponse.data;
              const docs = Array.isArray(docsData) ? docsData : docsData?.results || [];
              setPatientDocuments(docs);
              const docsTotalCount = docsData?.count || docsData?.total || docs.length;
              setDocumentsTotalCount(docsTotalCount);
            } catch (error) {
        ...
        <div className={`${isSidebarCollapsed ? 'w-10' : 'w-56 lg:w-64'} bg-white border-r border-slate-200 overflow-y-auto transition-all duration-300 shadow-sm scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-slate-100`}>
          {isSidebarCollapsed ? (
            ...
            <div className="p-1 space-y-2">
              ...
              <div className="space-y-1">
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  className="w-full h-7 hover:bg-amber-50 hover:text-amber-600 transition-colors p-0"
            ...
            <div className="p-2 space-y-3">
                ...
                <CardHeader className="pb-1.5 pt-2 px-2.5 bg-gradient-to-r from-emerald-50 via-emerald-50 to-teal-50 border-b border-slate-200">
                  <div className="flex items-center justify-between">
                    <CardTitle className="text-xs flex items-center gap-1.5 text-slate-800 font-semibold">
                      ...
                      Medical History
                      {medicalRecordsTotalCount > 0 && (
                        <span className="text-[9px] text-emerald-600 bg-emerald-100 px-1 py-0.5 rounded-full">
                          {medicalRecordsTotalCount}
                        </span>
                      )}
                    </CardTitle>
                    <Button
                    ...
                    {loadingRecords ? (
                      ...
                      <div className="space-y-1.5">
                        <div className="flex items-center justify-between">
                          ...
                          {medicalRecordsTotalCount > 3 && (
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => setShowAllMedicalRecords(!showAllMedicalRecords)}
                              className="h-4 text-[9px] px-1 py-0 text-emerald-600 hover:bg-emerald-100"
                            >
                              {showAllMedicalRecords ? 'Show Less' : 'View All'}
                            </Button>
                          )}
                        </div>
                        {(showAllMedicalRecords ? medicalRecords : medicalRecords.slice(0, 3)).map((record) => (
                          <div key={record.id} className="flex items-center justify-between p-1.5 bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200 rounded">
                            <div className="flex items-center gap-1.5 flex-1 min-w-0">
                              <div className="p-0.5 bg-emerald-100 rounded flex-shrink-0">
                                <FileText className="w-2.5 h-2.5 text-emerald-600" />
                              </div>
                              <div className="min-w-0 flex-1">
                                <p className="text-[10px] font-medium text-slate-800 truncate">{record.title}</p>
                        ...
                        {showAllMedicalRecords && medicalRecordsTotalCount > medicalRecordsPageSize && (
                          <div className="flex items-center justify-between pt-2 border-t border-slate-200">
                            <span className="text-[9px] text-slate-500">
                              Page {medicalRecordsPage} of {Math.ceil(medicalRecordsTotalCount / medicalRecordsPageSize)}
                            </span>
                            <div className="flex items-center gap-1">
                              <Button
                                ...
                                size="sm"
                                onClick={async () => {
                                  if (medicalRecordsPage > 1) {
                                    const newPage = medicalRecordsPage - 1;
                                    setMedicalRecordsPage(newPage);
                                    setLoadingRecords(true);
                                    try {
                                      const patientId = consultation?.patient?.id || consultation?.patient;
                                      const recordsResponse = await api.get(`/api/patients/${patientId}/medical-records/`, {
                                        params: { page: newPage, page_size: medicalRecordsPageSize }
                                      });
                                      const recordsData = recordsResponse.data.success ? recordsResponse.data.data : recordsResponse.data;
                                ...
                                disabled={medicalRecordsPage === 1 || loadingRecords}
                              ...
                              <Button
                                ...
                                size="sm"
                                onClick={async () => {
                                  if (medicalRecordsPage < Math.ceil(medicalRecordsTotalCount / medicalRecordsPageSize)) {
                                    const newPage = medicalRecordsPage + 1;
                                    setMedicalRecordsPage(newPage);
                                    setLoadingRecords(true);
                                    try {
                                ...
                                disabled={medicalRecordsPage >= Math.ceil(medicalRecordsTotalCount / medicalRecordsPageSize) || loadingRecords}
                                className="h-5 w-5 p-0"
                              >
                        ...
                        {!showAllMedicalRecords && medicalRecordsTotalCount > 3 && (
                          <div className="text-center py-1">
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => setShowAllMedicalRecords(true)}
                              className="text-[9px] text-emerald-600 bg-emerald-50 hover:bg-emerald-100 rounded-full px-2 py-0.5 h-auto"
                            >
                              +{medicalRecordsTotalCount - 3} more records - Click to view all
                            </Button>
                          </div>
                        )}
                      </div>
              ...
              {/* Patient Documents */}
              <Card className="border-slate-200 shadow-sm">
                <CardHeader className="pb-1.5 pt-2 px-2.5 bg-gradient-to-r from-amber-50 via-amber-50 to-orange-50 border-b border-slate-200">
                  <div className="flex items-center justify-between">
                    <CardTitle className="text-xs flex items-center gap-1.5 text-slate-800 font-semibold">
                      ...
                      {documentsTotalCount > 0 && (
                        <span className="text-[9px] text-amber-600 bg-amber-100 px-1 py-0.5 rounded-full">
                          {documentsTotalCount}
                        </span>
                      )}
                    </CardTitle>
                    ...
                    >
                      {showDocuments ? <ChevronDown className="w-3 h-3" /> : <ChevronRight className="w-3 h-3" />}
                    </Button>
                  </div>
                </CardHeader>
                {showDocuments && (
                  <CardContent className="bg-white p-2 space-y-2">
                    {loadingDocuments ? (
                      ...
                      <div className="space-y-1.5">
                        <div className="flex items-center justify-between">
                          ...
                          {documentsTotalCount > 3 && (
                            <Button
                              ...
                              className="h-4 text-[9px] px-1 py-0 text-amber-600 hover:bg-amber-100"
                            >
                              {showAllDocuments ? 'Show Less' : 'View All'}
                            </Button>
                          )}
                        </div>
                        {(showAllDocuments ? patientDocuments : patientDocuments.slice(0, 3)).map((doc) => (
                          <div key={doc.id} className="flex items-center justify-between p-1.5 bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded">
                            <div className="flex items-center gap-1.5 flex-1 min-w-0">
                              <div className="p-0.5 bg-amber-100 rounded flex-shrink-0">
                                <FileText className="w-2.5 h-2.5 text-amber-600" />
                              </div>
                              <div className="min-w-0 flex-1">
                                <p className="text-[10px] font-medium text-slate-800 truncate">{doc.title}</p>
                                <p className="text-[9px] text-slate-600">{doc.document_type?.replace('_', ' ')}</p>
                              </div>
                            </div>
                            <div className="flex items-center gap-0.5 flex-shrink-0">
                              {doc.file_url && (
                                <>
                                  <Button 
                                    ...
                                    onClick={() => {
                                      ...
                                      setSelectedRecordTitle(doc.title || 'Document');
                                      setRecordPreviewFullscreen(true);
                                      setShowRecordPreview(true);
                                    }}
                                    title="Preview"
                        ...
                        {!showAllDocuments && documentsTotalCount > 3 && (
                          <div className="text-center py-1">
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => setShowAllDocuments(true)}
                              className="text-[9px] text-amber-600 bg-amber-50 hover:bg-amber-100 rounded-full px-2 py-0.5 h-auto"
                            >
                              +{documentsTotalCount - 3} more documents - Click to view all
                            </Button>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="p-2 bg-slate-50 border border-slate-200 rounded">
                        <p className="text-[10px] text-slate-500 italic text-center">No documents uploaded</p>
                      </div>
                    )}
                  </CardContent>
                )}
