...
const PatientEditForm: React.FC<PatientEditFormProps> = ({
  ...
  const [userData, setUserData] = useState<Partial<CreatePatientUserData>>({
    phone: '',
    name: '',
    email: '',
    date_of_birth: '',
  ...
  const [profileData, setProfileData] = useState<Partial<CreatePatientProfileData>>({
    blood_group: '',
    allergies: '',
    chronic_conditions: [],
    current_medications: [],
  ...
  // Load patient data when form opens
  useEffect(() => {
    if (patient && open) {
      setUserData({
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Update user data
      const userUpdateData: Partial<CreatePatientUserData> = {};
      Object.entries(userData).forEach(([key, value]) => {
        if (value !== undefined && value !== '') {
          userUpdateData[key as keyof CreatePatientUserData] = value;
        }
      });

      // Update profile data
      const profileUpdateData: Partial<CreatePatientProfileData> = {};
      Object.entries(profileData).forEach(([key, value]) => {
        if (value !== undefined && value !== '') {
          profileUpdateData[key as keyof CreatePatientProfileData] = value;
        }
      });
      ...
      const updatedPatient: PatientProfile = {
        ...
        blood_group: updatedProfile.blood_group || patient.blood_group,
        allergies: updatedProfile.allergies || patient.allergies,
        chronic_conditions: updatedProfile.chronic_conditions || patient.chronic_conditions,
        current_medications: updatedProfile.current_medications || patient.current_medications,
        insurance_provider: updatedProfile.insurance_provider || patient.insurance_provider,
        insurance_policy_number: updatedProfile.insurance_policy_number || patient.insurance_policy_number,
        insurance_expiry: updatedProfile.insurance_expiry || patient.insurance_expiry,
        preferred_language: updatedProfile.preferred_language || patient.preferred_language
      };
      ...
      toast({
        title: "Error",
        description: error.response?.data?.error?.message || "Failed to update patient. Please try again.",
        variant: "destructive"
      });
    } finally {
      setIsLoading(false);
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold flex items-center gap-2">
            ...
            Edit Patient: {patient.user_name}
          </DialogTitle>
          <DialogDescription>
            Update the patient's information. All fields marked with * are required.
          </DialogDescription>
        </DialogHeader>
