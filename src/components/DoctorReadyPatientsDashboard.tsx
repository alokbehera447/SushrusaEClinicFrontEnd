...
interface DoctorReadyPatientsDashboardProps {
  onConsultationSelect?: (consultation: Consultation) => void;
}

export const DoctorReadyPatientsDashboard: React.FC<DoctorReadyPatientsDashboardProps> = ({
  ...
  const loadReadyPatients = async () => {
    try {
      setLoading(true);
      const response = await consultationService.getReadyPatients();
      
      if (response.success) {
        setReadyPatients(response.data);
      } else {
        toast({
          title: "Error",
  ...
  const handleStartConsultation = async (consultation: Consultation) => {
    try {
      ...
      if (response.success) {
        ...
        loadReadyPatients(); // Refresh the list
    ...
    <div className="space-y-6">
        ...
        <CardContent className="p-6">
          <div className="flex items-center space-x-4">
            <div className="flex items-center space-x-2">
              ...
              <div>
                <p className="text-sm font-medium text-gray-600">Ready Patients</p>
                <p className="text-3xl font-bold text-green-600">{readyPatients.length}</p>
              </div>
            </div>
      ...
      <Card className="border-0 shadow-lg">
        <CardHeader>
          ...
          <CardDescription>
            Click "Start Consultation" to begin the consultation process
          </CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <div className="flex items-center justify-center space-x-2 text-sm text-gray-400">
                ...
                <span>Check with admin to ensure patients are properly checked in</span>
              </div>
            </div>
          ) : (
                ...
                <Button
                  onClick={() => {
                    if (readyPatients.length > 0) {
                      handleStartConsultation(readyPatients[0]);
                    }
                  }}
